...
describe('InMemoryOutboxStore', () => {
  it('enqueues and pulls pending records', () => {
    ...
    const enqueue = store.enqueue('forms:contact:sub_123', { event: 'forms.submission.received' });
  ...
  it('updates status and attempt count transitions', () => {
    const store = new InMemoryOutboxStore();
    const record = store.enqueue('forms:contact:sub_123', { event: 'forms.submission.received' }).record;
  ...
  it('dedupes duplicate enqueue and tracks sent ledger', () => {
    const store = new InMemoryOutboxStore();
    const first = store.enqueue('forms:contact:sub_123', { event: 'forms.submission.received' });
    const second = store.enqueue('forms:contact:sub_123', { event: 'forms.submission.received' });
    expect(first.deduped).toBe(false);
    expect(second.deduped).toBe(true);

    store.markSent(first.record!.id);
    expect(store.isEventSent('forms:contact:sub_123')).toBe(true);
    expect(store.getStats().sentLedgerCount).toBe(1);
  });
});
